...
var games_8_per_genre = games.Where(g => g.Beoordeling >= 8).GroupBy(g => g.Genre).ToDictionary(k => k.Key, v => v.ToList());

foreach (var groep in games_8_per_genre)
{
    Console.WriteLine($"{groep.Key}: {string.Join(", ", groep.Value.Select(g => g.Titel))}");
}